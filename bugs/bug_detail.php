...
$sql = sprintf("select wybug_id from record where wybug_id = '%s'",mysql_real_escape_string($_GET['wybug_id']));
...
<div class="wxewm">
<a class="ewmthumb" href="javascript:void(0)"><span></span></a>
		</div>
		</div>
				<div class="login">
									<?php
	...
	<div class="nav" id="nav_sc">
			...
			<li><a href="/" style="color:rgb(246,172,110);font-size:14px;font-weight:blod">乌云漏洞库</a></li>
			<li><a>公告</a></li>
		</ul>
...
<input type="hidden" id="token" style="display:none" value="" />
		<h2>漏洞概要
		<span style="margin:0 0 0 580px; float:right; position:absolute; font-size:14px; font-weight:normal">
		<?php
			if($record_detail['wybug_id']){
			  echo "<b><a class='btn' style='color:red'>漏洞已阅<a></b>";		
			} else{
			  echo "<span id='attend_action'><a id='record_wybug_id' class='btn' href='record.php?wybug_id=".$bug_detail['wybug_id']."'>记录当前未学习漏洞</a></span><script>document.getElementById('record_wybug_id').onclick=function(){window.location.reload();};	</script>";
			}
		?>
...
$(".btn-fav").click(function(){ CollectBug(id,token); });
		...
		<?php echo $bug_detail['wybug_detail']; ?>

		<hr align="center" />

		<?php echo $bug_detail['wybug_reply']; ?>
...
$(document).ready(function(){
	...
	<div id="footer">
        <span class="copyright fleft">
    		Copyright &copy; 2010 - 2016 <a href="#">#</a>, All Rights Reserved。 <span style='color:red'>Welcome To WeiyiGeek's blog <a href="https://weiyigeek.top">https://weiyigeek.top</a></span>
    	</span>
